...
namespace MonoGameWindowsStarter
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (ballPosition.X == playerRect.X + playerRect.Width)
            {
                ballVelocity.X *= -1;
                float delta = 0 - ballPosition.X;
                ballPosition.X += 2 * delta;
            }
            if (ballPosition.X + ballRect.Width < playerRect.X)
            {
                ballVelocity.X *= -1;
                float delta = 0 - ballPosition.X;
                ballPosition.X += 2 * delta;
            }
            if (ballPosition.Y < playerRect.Y + playerRect.Height)
            {
                ballVelocity.Y *= -1;
                float delta = 0 - ballPosition.Y;
                ballPosition.Y += 2 * delta;
            }
            if (ballPosition.Y + ballRect.Height > playerRect.Y)
            {
                ballVelocity.Y *= -1;
                float delta = 0 - ballPosition.Y;
                ballPosition.Y += 2 * delta;
